...
const Login = () => {
    const { login, storeUser } = useAuth(); // Access isAuthenticated and login from AuthContext
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setLoading(true)
            const response = await axios.post(`${process.env.REACT_APP_SERVER_IP_ADDRESS}/api/users/login-user`, {
                email: email,
                password: password
            });
            ...
            if (error?.response && error?.response?.data && error?.response?.data?.response?.message) {
                setValidationErrors([error?.response?.data?.response?.message]);
                // Scroll to the error message container if it exists
